...
- API: `mcp_editor/tool_editor_web.py`
  - `generate_server_from_web()`에서 경로 결정 후 `jinja/generate_server.py` 실행
...
- 기본값(`_get_template_for_server`) 매핑
  - `jinja/outlook_server_template.jinja2` (server_name = outlook)
  - `jinja/file_handler_server_template.jinja2` (server_name = file_handler)
  - 그 외: `jinja/mcp_server_scaffold_template.jinja2` 또는 존재하는 템플릿 중 첫 번째
- `jinja/universal_server_template.jinja2`는 필요 시 모달에서 직접 지정 가능

### 레지스트리(서비스 메타데이터) 파일
- `jinja/generate_server.py`가 registry를 자동 탐색
...
- `tool_internal_args.json`은 생성 시점에는 사용되지 않지만,
  ...
  - 예: `jinja/outlook_server_template.jinja2`는 `tool_internal_args.json` 로드 로직 포함
