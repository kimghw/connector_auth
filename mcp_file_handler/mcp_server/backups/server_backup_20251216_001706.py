...
async def handle_tool_call(request: MCPRequest) -> JSONResponse:
    ...
    try:
        ...
        # Route to appropriate handlerif tool_name == "convert_file_to_text":
            result = await handle_convert_file_to_text(arguments)elif tool_name == "convert_onedrive_to_text":
            result = await handle_convert_onedrive_to_text(arguments)elif tool_name == "process_directory":
            result = await handle_process_directory(arguments)elif tool_name == "save_file_metadata":
            result = await handle_save_file_metadata(arguments)elif tool_name == "search_metadata":
            result = await handle_search_metadata(arguments)elif tool_name == "get_file_metadata":
            result = await handle_get_file_metadata(arguments)elif tool_name == "delete_file_metadata":
            result = await handle_delete_file_metadata(arguments)        else:
            return create_error_response(
                request.id,
                -32602,
