...
async def handle_tool_call(request: MCPRequest) -> JSONResponse:
    ...
    try:
        ...
        # Route to appropriate handler (only defined tools)
...
async def handle_mail_search(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Convert dicts to parameter objects
    filter_params = FilterParams(**client_filter) if client_filter else None
    select_params = SelectParams(**select_dict) if select_dict else None

    try:
        query_instance = get_query_instance(context)
        return await query_instance.query_search(
            user_email=user_email,
            search=search,
            client_filter=filter_params,
            select=select_params,
