...
- `/setup-auth-graphapi` → 기본 OAuth 플로우 실행
- `/setup-auth-graphapi --force` → `.env`가 이미 있어도 새로 입력받아 덮어쓰기
...
- `AADSTS7000215` (invalid client secret) → Client Secret 값을 다시 입력받기 (`/setup-auth-graphapi --force`로 재실행 안내)
...
**입력**: `/setup-auth-graphapi --force` (기존 .env 덮어쓰기)
...
**입력**: `/setup-auth-graphapi` (코드가 다른 DB 파일명을 사용하는 경우)
