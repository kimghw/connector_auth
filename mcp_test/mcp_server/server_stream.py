"""
Streaming MCP Server for Test MCP Server
...
server_module_dir = os.path.join(grandparent_dir, "mcp_test")
...
def _load_mcp_tools() -> List[Dict[str, Any]]:
    ...
    2. mcp_editor/mcp_{profile_name}/tool_definition_templates.yaml (profile-specific)
       - Uses test which is set correctly at generation time for reused profiles
    ...
    if yaml_path_str:
        ...
        yaml_path = Path(current_dir).parent.parent / "mcp_editor" / "mcp_test" / "tool_definition_templates.yaml"
...
class StreamableHTTPMCPServer:
    ...
    def __init__(self):
        ...
        self.setup_routes()
        logger.info(f"Test MCP Server StreamableHTTP Server initialized")
    ...
    async def on_startup(self, app):
        """서버 시작 시 실행"""
        logger.info(f"Test MCP Server StreamableHTTP Server starting on port {app['port']}")
    ...
    async def on_cleanup(self, app):
        """서버 종료 시 정리"""
        logger.info(f"Test MCP Server StreamableHTTP Server shutting down")
    ...
    def run(self, host: str = '0.0.0.0', port: int = 8001):
        ...
        logger.info(f"Starting Test MCP Server StreamableHTTP Server on {host}:{port}")
