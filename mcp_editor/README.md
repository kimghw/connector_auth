...
웹 에디터 UI (브라우저)
  ↓ POST /api/tools
웹 에디터 백엔드 (tool_editor_web.py:284 save_tool_definitions)
  ↓
┌─────────────────────────────────────────────────────────────┐
│ 1️⃣ 데이터 파싱 및 정제 (tool_editor_web.py에서 처리)        │
│   ├─ tools_data (웹에서 받은 JSON)                          │
│   ├─ mcp_service 메타데이터 유지                            │
│   └─ inputSchema 필드 순서 정리                             │
└─────────────────────────────────────────────────────────────┘
  ↓
┌─────────────────────────────────────────────────────────────┐
│ 2️⃣ 소스코드 스캔 (AST 파싱)                                 │
...
│       if tool['mcp_service']['name'] in signatures_by_name: │
│         tool['mcp_service']['signature'] = signatures[...]  │
│     ↓                                                       │
│     저장 내용:                                              │
│     {                                                       │
│       "name": "query_emails",                               │
...
│       "mcp_service": {  ← 메타데이터 유지                   │
│         "name": "query_search",  ← 실제 함수명             │
│         "signature": "user_email: str, ..."  ← AST 추출    │
│       }                                                     │
│     }                                                       │
│     ↓                                                       │
│     용도: 웹 에디터 로드, server.py 생성 시 사용            │
│                                                             │
...
│         "inputSchema": remove_defaults(tool["inputSchema"]) │
│         # mcp_service, mcp_service_factors 제거 ←           │
│       }                                                     │
│     ↓                                                       │
│     저장 내용:                                              │
│     {                                                       │
│       "name": "query_emails",                               │
│       "description": "...",                                 │
│       "inputSchema": {...}  ← default 값 제거              │
│       # mcp_service 필드 없음                               │
│     }                                                       │
│     ↓                                                       │
│     용도: MCP 프로토콜, Claude/OpenAI API 전송용            │
└─────────────────────────────────────────────────────────────┘
```

**핵심 포인트:**
- 모든 데이터 파싱은 `tool_editor_web.py`의 `save_tool_definitions()` 함수에서 처리
- `templates.py`는 순수 데이터 파일 (파싱 로직 없음, JSON만 저장)
- AST 파싱은 저장 시점에 실시간으로 수행 (코드베이스 스캔)
- 두 파일의 차이: 메타데이터 포함 여부만 다름

### server.py 생성

```
사용자 실행 (jinja/generate_outlook_server.py --replace)
  ↓
┌─────────────────────────────────────────────────────────────┐
│ 1️⃣ 템플릿 파일 로드 (generate_outlook_server.py:476)       │
│   tool_definition_outlook_templates.py                      │
│   ↓                                                         │
│   import로 Python 모듈 동적 로드                            │
│   ↓                                                         │
│   MCP_TOOLS = [                                             │
│     {                                                       │
│       "name": "query_emails",                               │
...
└─────────────────────────────────────────────────────────────┘
  ↓
┌─────────────────────────────────────────────────────────────┐
│ 2️⃣ 툴 분석 및 파라미터 파싱 (generate_outlook_server.py)   │
│   analyze_tool_parameters()                                 │
│   ↓                                                         │
│   for tool in MCP_TOOLS:                                    │
│     # mcp_service.signature 파싱                            │
│     signature = tool['mcp_service']['signature']            │
│     params = parse_signature(signature)                     │
│     ↓                                                       │
│     # inputSchema와 비교하여 타입 추론                      │
│     if param_type == 'object':                              │
│       # Pydantic 모델로 변환 필요                           │
│       object_params['filter'] = 'FilterParams'              │
│     ↓                                                       │
│     analyzed_tools[tool['name']] = {                        │
│       'service_method': 'query_filter',  ← 호출할 함수      │
│       'params': {...},                  ← 일반 파라미터     │
│       'object_params': {...},           ← 객체 파라미터     │
│       'call_params': {...}              ← 함수 호출 매핑    │
│     }                                                       │
└─────────────────────────────────────────────────────────────┘
  ↓
┌─────────────────────────────────────────────────────────────┐
...
│   - param_types: import할 Pydantic 타입                    │
│   - tools_data: 분석된 툴 정보                             │
│   ↓                                                         │
│   {% for tool in tools_data %}                              │
│   if tool_name == "{{ tool.name }}":                        │
│       {% for param, info in tool.object_params.items() %}   │
│       {{ param }}_params = {{ info.class_name }}(**args["{{ param }}"])│
│       {% endfor %}                                          │
│       result = service.{{ tool.service_method }}(           │
│           {% for param in tool.call_params %}               │
│           {{ param }}={{ param }},                          │
│           {% endfor %}                                      │
│       )                                                     │
│   {% endfor %}                                              │
│   ↓                                                         │
│   렌더링된 Python 코드 (server.py):                         │
│   if tool_name == "query_emails":                           │
│       filter_params = FilterParams(**args.get("filter", {}))│
│       result = service.query_filter(                        │
...
│   ↓                                                         │
│   --replace 옵션이면:                                       │
│   - 기존 server.py를 backups/ 폴더에 백업                  │
│   - 새 server.py로 덮어쓰기                                 │
└─────────────────────────────────────────────────────────────┘
```

**핵심 포인트:**
- `templates.py`의 `mcp_service.signature`를 파싱하여 함수 호출 코드 생성
- Jinja2 템플릿이 Python 코드를 생성 (메타프로그래밍)
- `inputSchema`의 `object` 타입은 자동으로 Pydantic 모델로 변환
- 모든 파싱과 분석은 `generate_outlook_server.py`에서 처리
- 템플릿은 순수 Jinja2 문법 (로직 없음)
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│ 1. 웹 에디터 저장 시 (tool_editor_web.py:284)                  │
│    ✓ 웹에서 받은 JSON 파싱                                      │
│    ✓ AST로 소스코드 스캔 (mcp_service.signature 추출)          │
│    ✓ 2개 파일 생성 (templates.py, tool_definitions.py)         │
│    ✗ templates.py는 데이터만 저장 (파싱 로직 없음)             │
│                                                                 │
│ 2. server.py 생성 시 (generate_outlook_server.py)              │
│    ✓ templates.py 로드 (Python import)                         │
│    ✓ mcp_service.signature 파싱                                 │
│    ✓ inputSchema 분석 및 타입 추론                              │
│    ✓ Jinja2 템플릿 렌더링 (Python 코드 생성)                   │
│    ✗ templates.py 수정하지 않음 (읽기만)                       │
│                                                                 │
│ 3. 런타임 (server.py 실행 시)                                  │
│    ✓ tool_definitions.py 로드                                  │
│    ✓ MCP 프로토콜로 Claude/OpenAI에 툴 정의 전송                │
│    ✗ 파싱 없음 (이미 생성된 코드 실행)                         │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘

**결론: templates.py는 순수 데이터 저장소**
- 파싱은 tool_editor_web.py와 generate_*_server.py에서만 발생
- templates.py는 중간 데이터 형식 (메타데이터 포함)
- tool_definitions.py는 최종 결과 (API 전송용)
```
...
# 2. 브라우저에서 "Create New Server" 버튼 클릭
#    - 서버 이름 입력 (예: calendar)
#    - 설명 입력
#    - 포트 번호 설정
#    - Create 버튼 클릭

# 3. 생성 완료 후 자동으로 프로필 추가됨
#    - editor_config.json에 새 프로필 추가
#    - mcp_calendar/ 폴더 구조 생성
#    - tool_definition_calendar_templates.py 생성
```
...
- 새 서버 생성 시 자동으로 필요한 모든 파일이 생성됩니다:
  - `mcp_{server_name}/mcp_server/` 디렉토리 구조
  - `server.py`, `tool_definitions.py`, `run.py`
  - `tool_definition_{server_name}_templates.py`
  - Jinja2 템플릿 파일
  - `editor_config.json`에 프로필 추가
