...
- `../outlook_mcp/mcp_server/tool_definitions.py`: 런타임용 정리된 정의(내부 메타 제거, 기본값 제거). 저장 시 템플릿 파일과 함께 자동 생성.
...
- `editor_config.json`: 에디터 경로 설정 파일. 기본값으로 생성되며 아래 경로 키를 가진다:
  ...
  - `tool_definitions_path`: 런타임 도구 정의 파일 경로 (기본: `../outlook_mcp/mcp_server/tool_definitions.py`)
  - `backup_dir`: 백업 디렉터리 경로 (기본: `backups`)
  - `graph_types_files`: graph_types.py 위치 목록 (복수 지원, 기본: `["../outlook_mcp/graph_types.py"]`)
  - `host`, `port`: 에디터 서버 바인드 설정 (기본: `127.0.0.1`, `8091`)
- 프로필 다중 지원: `editor_config.json`에 `_default`, `outlook` 등 프로필을 여러 개 넣을 수 있음. 프런트엔드에서 탭 형태로 선택/전환 가능. 서버는 `MCP_EDITOR_CONFIG`(파일 경로) 또는 `MCP_EDITOR_MODULE=<프로필명>`으로 프로필을 선택할 수 있으며, 기본은 `_default`.

저장 시 생성/갱신:
- `tool_definition_templates.py`와 `../outlook_mcp/mcp_server/tool_definitions.py` 둘 다 업데이트.
- `backups/`에 백업 파일 추가.
