...
def compare_structures(original, refactored):
    ...
    if original['sidebar'] and refactored['sidebar']:
        print(f"  Tool List: Original={original['sidebar']['tool_list']}, Refactored={refactored['sidebar']['tool_list']}")
    ...
    only_original = original['classes'] - refactored['classes']
    only_refactored = refactored['classes'] - original['classes']

    print(f"  Common:     {len(common_classes)} classes")

    # 주요 클래스 체크
    important_classes = [
        'container', 'header', 'sidebar', 'tool-list',
        'editor-area', 'modal', 'btn', 'form-control'
    ]
